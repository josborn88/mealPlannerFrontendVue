<template>
  <div class="hello">
    ...
    <div class="fm-data">
      ...
      <input type="text" id="lastMadeDate" placeholder="YYYY-MM-DD"
      class="fm-txt" v-model="lastMade"/>
    </div>
    <div class="btn-block">
    ...
    <div class="btn-block">
      <div v-if="randomMeal">
        <button v-on:click.prevent="makeMeal(randomMeal.id)"
          class="btn">
          Make this meal
        </button>
...
export default {
  ...
  methods: {
    ...
    makeMeal(id) {
      axios
        .get(`http://localhost:8000/makeMeal/${id}`)
        .then((response) => {
          console.log(response.data);
        });
    },
